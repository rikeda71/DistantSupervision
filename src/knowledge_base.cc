...
knowledge_base::words knowledge_base::split(
    const std::string &str, const char &deliminator) {
...
void knowledge_base::KnowledgeBaseRE::load() {
  ...
  while (getline(ifs, str)) {
    ...
    this->knowledges.insert(
        std::make_tuple(splited[0], splited[1], splited[2]));
  }
}
